...
const FilterMenu = ({setWordFreqs} :  FilterMenuProps) => {    
    const [gotSources, setGotSources] = useState(false)
    ...
    async function getWordFreqs(sourcesToFetch) {
      ...
      let test = "Here's".replace(/^[^\w\s]+|[^\w\s]+$/g, '').toLowerCase()
      // console.log("Here's".replace(/^[^\w\s]+|[^\w\s]+$/g, '').toLowerCase())
      console.log(stopwords.has(test))
      ...
      let toDisplay: WordCount[] = Array.from(wordFreqs.entries())
      toDisplay.sort((a, b) => b[1] - a[1]);
      toDisplay = toDisplay.slice(0, 200)
